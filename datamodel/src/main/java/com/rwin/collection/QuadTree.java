...
public class QuadTree<E> {
    ...
    public QuadTree(double minX, double minY, double maxX, double maxY) {
        this.root_ = new Node<E>(minX, minY, maxX - minX, maxY - minY, null);
    }
     ...
     */
    public void set(double x, double y, E value) {

        Node<E> root = this.root_;
        if (x < root.getX() || y < root.getY() || x > root.getX() + root.getW()
                || y > root.getY() + root.getH()) {
            throw new QuadTreeException("Out of bounds : (" + x + ", " + y
                    + ")");
        }
        if (this.insert(root, new Point<E>(x, y, value))) {
            this.count_++;
        }
    }
     ...
     *         been provided.
     */
    public E get(double x, double y, E opt_default) {
        Node<E> node = this.find(this.root_, x, y);
     ...
     *         node doesn't exist.
     */
    public E remove(double x, double y) {
        Node<E> node = this.find(this.root_, x, y);
        if (node != null) {
            E value = node.getPoint().getValue();
            node.setPoint(null);
     ...
     */
    public E[] getValues() {
        final List<E> arr = new ArrayList<E>();
        this.traverse(this.root_, new Func<E>() {
            @Override
    ...
    public List<Point> searchIntersect(final double xmin, final double ymin,
            final double xmax, final double ymax) {
        final List<Point> arr = new ArrayList<Point>();
        this.navigate(this.root_, new Func<E>() {
            @Override
            public void call(QuadTree<E> quadTree, Node<E> node) {
                Point<E> pt = node.getPoint();
                if (pt.getX() < xmin || pt.getX() > xmax || pt.getY() < ymin
                        || pt.getY() > ymax) {
    ...
    public List<Point<E>> searchWithin(final double xmin, final double ymin,
            final double xmax, final double ymax) {
        final List<Point<E>> arr = new ArrayList<Point<E>>();
        this.navigate(this.root_, new Func<E>() {
            @Override
            public void call(QuadTree<E> quadTree, Node<E> node) {
                Point<E> pt = node.getPoint();
    ...
    public void navigate(Node<E> node, Func<E> func, double xmin, double ymin,
    ...
    private boolean intersects(double left, double bottom, double right,
            double top, Node node) {
        return !(node.getX() > right || (node.getX() + node.getW()) < left
    ...
    public QuadTree clone() {
        ...
        this.traverse(this.root_, new Func() {
            @Override
            public void call(QuadTree quadTree, Node node) {
                clone.set(node.getPoint().getX(), node.getPoint().getY(), node
                        .getPoint().getValue());
            }
        });
     ...
     * @private
     */
    public void traverse(Node<E> node, Func<E> func) {
        switch (node.getNodeType()) {
        case LEAF:
            func.call(this, node);
            break;
     ...
     * @private
     */
    public Node<E> find(Node<E> node, double x, double y) {
        Node<E> resposne = null;
        switch (node.getNodeType()) {
        case EMPTY:
            break;

        case LEAF:
            resposne = node.getPoint().getX() == x
                    && node.getPoint().getY() == y ? node : null;
            break;
     ...
     * @private
     */
    private boolean insert(Node<E> parent, Point<E> point) {
        ...
        case EMPTY:
            ...
            break;
        case LEAF:
            if (parent.getPoint().getX() == point.getX()
                    && parent.getPoint().getY() == point.getY()) {
                this.setPointForNode(parent, point);
                result = false;
            } else {
                this.split(parent);
            ...
            break;
        case POINTER:
            result = this.insert(this.getQuadrantForPoint(parent, point.getX(),
                    point.getY()), point);
            break;
    ...
    private void split(Node<E> node) {
        Point<E> oldPoint = node.getPoint();
        node.setPoint(null);
        ...
        node.setNw(new Node<E>(x, y, hw, hh, node));
        node.setNe(new Node<E>(x + hw, y, hw, hh, node));
        node.setSw(new Node<E>(x, y + hh, hw, hh, node));
        node.setSe(new Node<E>(x + hw, y + hh, hw, hh, node));
    ...
    private void balance(Node<E> node) {
        ...
        case POINTER: {
            Node<E> nw = node.getNw();
            Node<E> ne = node.getNe();
            Node<E> sw = node.getSw();
            Node<E> se = node.getSe();
    ...
    private void setPointForNode(Node node, Point point) {
        if (node.getNodeType() == NodeType.POINTER) {
            throw new QuadTreeException(
                    "Can not set point for node of type POINTER");
        }
        node.setNodeType(NodeType.LEAF);
        node.setPoint(point);
    }
